...
(() => {
  ...
  function mountGallery(){
    ...
    const render=()=>{ const w=works[current]; art.style.setProperty('--x',w[2]); art.style.setProperty('--y',w[3]); idx.textContent=String(current+1).padStart(2,'0'); h.textContent=w[0]; p.textContent=w[1]+' / ORLICA TATT'; viewer.querySelector('.work-enquiry').href='https://t.me/Sveta_orel09?text='+encodeURIComponent(`Привет, Света! Понравилась работа ${current+1} «${w[0]}» в твоём портфолио. Хочу обсудить татуировку в таком настроении: https://wwgame28.github.io/Tattoo-/#works`); };
    const open=(i)=>{clearTimeout(closeTimer);returnFocus=document.activeElement;current=i;render();viewer.style.display='grid';requestAnimationFrame(()=>viewer.classList.add('is-open'));viewer.setAttribute('aria-hidden','false');document.body.classList.add('viewer-open');viewer.querySelector('.orlica-viewer-close').focus();};
    const close=()=>{viewer.classList.remove('is-open');viewer.setAttribute('aria-hidden','true');document.body.classList.remove('viewer-open');returnFocus?.focus({preventScroll:true});closeTimer=setTimeout(()=>viewer.style.display='none',280);};
    ...
    // Swipe only on the artwork; vertical movement continues to scroll on phones.
    const media=viewer.querySelector('.orlica-viewer-media');let swipe=null;
    media.addEventListener('pointerdown',e=>{if(e.isPrimary)swipe={x:e.clientX,y:e.clientY,id:e.pointerId};});
    media.addEventListener('pointerup',e=>{if(!swipe||swipe.id!==e.pointerId)return;const dx=e.clientX-swipe.x,dy=e.clientY-swipe.y;swipe=null;if(Math.abs(dx)>50&&Math.abs(dx)>Math.abs(dy)*1.5){current=(current+(dx<0?1:works.length-1))%works.length;render();}});
    media.addEventListener('pointercancel',()=>{swipe=null;});
    viewer.addEventListener('keydown',e=>{if(e.key!=='Tab')return;const focusable=[...viewer.querySelectorAll('button,a[href]')];const first=focusable[0],last=focusable.at(-1);if(e.shiftKey&&document.activeElement===first){e.preventDefault();last.focus();}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first.focus();}});
  ...
  async function getSlots(){
    ...
    const r=await fetch(`${API_URL}/rest/v1/booking_slots?select=id,starts_at,duration_minutes&status=eq.open&starts_at=gt.${now}&order=starts_at.asc&limit=80`,{headers:headers(),cache:'no-store',signal:AbortSignal.timeout(10000)});
  ...
  function mountBooking(){
    ...
    const render=()=>{
      sec.dataset.availability=slots.length?'open':'empty';
      if(!slots.length) emptyEl.textContent='Свободных окон пока нет. Напиши Свете в Telegram по кнопке выше или оставь заявку в листе ожидания.';
    ...
    const load=async()=>{try{slots=await getSlots();render();}catch(e){sec.dataset.availability='error';copyEl.textContent='Календарь временно недоступен — напиши Свете';emptyEl.hidden=false;emptyEl.innerHTML='Не удалось загрузить даты. Обсуди удобное время в Telegram по кнопке выше.';}};
  ...
  function mountCalendar(rows){
    ...
    const render=()=>{
      ...
      for(let d=1;d<=daysInMonth;d++){
        ...
        const label=state==='open'?`${count} ${count===1?'окно':'окна'}`:state==='past'?'прошло':'нет записи';
