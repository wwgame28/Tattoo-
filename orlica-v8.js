(() => {
  ...
  function mountBooking(){
    ...
    form.addEventListener('submit',async e=>{
      ...
      try{
        const r=await fetch(`${API_URL}/rest/v1/rpc/submit_booking`,{method:'POST',headers:{...headers(),'Prefer':'return=representation'},body:JSON.stringify({p_slot_id:selectedSlot.id,p_client_name:String(fd.get('name')||'').trim(),p_contact:contact,p_telegram:contact.startsWith('@')?contact:null,p_tattoo_place:String(fd.get('place')||'').trim(),p_tattoo_size:String(fd.get('size')||'').trim(),p_idea:String(fd.get('idea')||'').trim(),p_reference_url:String(fd.get('reference')||'').trim()||null})});
        const data=await r.json().catch(()=>null); if(!r.ok) throw new Error(data?.message?.includes('SLOT_UNAVAILABLE')?'Это окно только что заняли. Обновляю календарь…':(data?.message||'Не удалось отправить заявку'));
        const row=Array.isArray(data)?data[0]:data; form.style.display='none';success.classList.add('show');sec.querySelector('#booking-code').textContent=row?.booking_code||'OK';sec.querySelector('#booking-success-text').textContent=`${prettyFull(row?.starts_at||selectedSlot.starts_at)}. Света подтвердит запись после просмотра идеи.`;await load();
