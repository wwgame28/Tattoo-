(() => {
  ...
  const ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJIUzI1NiIsInJlZiI6ImRid252YmZkcGhxbWZqemJwcW53Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODk2ODYxNzIsImV4cCI6MjEwNTI2MjE3Mn0.3fxOPVcWuVCMmACMCkRR9vjt2SLMts-DjI2SSuYof5A';
  ...
  function mountBooking(){
    ...
    form.addEventListener('submit',async e=>{
      ...
      try{
        const r=await fetch(`${API_URL}/functions/v1/orlica-book`,{method:'POST',headers:headers(),body:JSON.stringify({slot_id:selectedSlot.id,client_name:String(fd.get('name')||'').trim(),contact,telegram:contact.startsWith('@')?contact:null,tattoo_place:String(fd.get('place')||'').trim(),tattoo_size:String(fd.get('size')||'').trim(),idea:String(fd.get('idea')||'').trim(),reference_url:String(fd.get('reference')||'').trim()||null,website:String(fd.get('website')||'')})});
        const data=await r.json().catch(()=>null); if(!r.ok){const code=data?.error||'';throw new Error(code==='SLOT_UNAVAILABLE'?'Это окно только что заняли. Обновляю календарь…':code==='RATE_LIMIT'?'Слишком много попыток. Попробуй немного позже.':'Не удалось отправить заявку');}
        const row=data||{}; form.style.display='none';success.classList.add('show');sec.querySelector('#booking-code').textContent=row?.booking_code||'OK';sec.querySelector('#booking-success-text').textContent=`${prettyFull(row?.starts_at||selectedSlot.starts_at)}. Света подтвердит запись после просмотра идеи.`;await load();
      }catch(err){showMessage(err.message);await load();submit.disabled=!selectedSlot;submit.firstChild.textContent='Отправить заявку ';}
    });
